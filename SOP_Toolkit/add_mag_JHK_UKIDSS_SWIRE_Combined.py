...
if len(argv) != 6:
    exit('\n\tWrong Input Argument!\
        \n\tExample: python [program] [SWIRE] [UKIDSS] [HEADER_LENGTH] [output file name] [Optional-Conditions]\
...
for i in range(len(UK_cat_origin)):
    ...
    if float((REPT[j].split(','))[6]) != 0.0 and float((REPT[j].split(','))[7]) != 0.0:
    ...
    # If No Detection => Remain 2MASS catalog
    else:
        row_s[35], row_s[56], row_s[77] = JHK_flux_to_mag(row_s[33], row_s[54], row_s[75])
