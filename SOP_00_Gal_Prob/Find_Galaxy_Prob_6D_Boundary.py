...
if len(argv) != 10:
    ...
    \n\t[lower_bd]: bound of input bands except fixed one e.g. [0,0,0,0,0] unit:cell\
    \n\t[upper_bd]: bound of input bands except fixed one e.g. [9,9,9,9,9] unit:cell\n')
...
sc_lower_bd = [int(i) for i in argv[8]] #
sc_upper_bd = [int(j) for j in argv[9]] #
...
tomo_dir = '{}_GPtomo/'.format(out_dir)
pca_dir  = '{}/pca_cut/'.format(tomo_dir)

# Functions
#==========================================================
def generate_origins_on_plane(origin, band_inp, sc_fixed_bd, sc_lower_bd, sc_upper_bd):
    '''
    ...
    origin_list = []
    x0, x1, x2, x3, x4 = np.meshgrid(bd_list[0], bd_list[1], bd_list[2], bd_list[3], bd_list[4])
    ...
    for j in range(len(x0.flatten())):
        ...
        new_origin.insert(sc_fixed_bd, origin[sc_fixed_bd])
...
def generate_pca_line(origin, pca_vec, band_lower_bd, band_upper_bd):
    ...
    if not np.all(np.less_equal(pca_vec, band_lower_bd)):
        pca_vec = -1 * pca_vec
    # Prevent infinite loops
    if np.any(np.greater_equal(pca_vec, band_lower_bd)):
    ...
    while np.all(np.greater_equal(pos, band_lower_bd)):
        ...
        i += 1
    # Larger than origin
    pca_vec, pos, j = -1 * pca_vec, origin, 1
...
def find_gp_boundary(pca_line, gp_along_line):
    ...
    '''
    loc_LS1_id = [gp_along_line <  1.0]
    loc_GE1_id = [gp_along_line >= 1.0]
    lower_gp_bound = pca_line[loc_LS1_id[0] + 1]
    upper_gp_bound = pca_line[loc_GE1_id[-1]]
...
if __name__ == '__main__':
    ...
    pca_var0       = np.load('{}/PCA_var_ratios_{}.npy'.format(pca_dir, band_inp))[0]
    ...
    #============================================================
    #TODO Use parallel by joblib package and run on fomalhaut ...
    #============================================================

    # Use different origins to find boundaries
    gp_lower_bd_list, gp_upper_bd_list = [], []
    origin_list = generate_origins_on_plane(pca_origin, band_inp, sc_fixed_bd, sc_lower_bd, sc_upper_bd)
    for i, origin in enumerate(origin_list):
        ...
        gp_upper_bd, gp_lower_bd = find_gp_boundary(pca_line, gp_along_line)
    ...
    chdir(out_dir)
    np.save('after_smooth_{:d}D_lower_bounds'.format(dim), gp_lower_bounds)
    np.save('after_smooth_{:d}D_upper_bounds'.format(dim), gp_upper_bounds)
    chdir('../')
