...
# Main Functions
#==========================================================
def generate_probe(origin, shape, along_bd_id):
    '''
    This is to generate probe array along the band with most cells
    '''
    probe_array = np.array([origin for _ in range(shape[along_bd_id])])
    probe_array[:, along_bd_id] = np.arange(0, shape[along_bd_id], 1)
    probe_array = np.array(probe_array, dtype=int)
    return probe_array
...
def execute_all(origin_pos):
    # Assign titles and savefig names
    indice = ['{:0>3d}'.format(origin_pos[j]) for j in range(len(origin_pos)) if j != along_bd_id]
    svname = '{}{:d}_{}'.format(cut_style, along_bd_id, '_'.join(indice))
    title  = '{} smooth_bond={:d}'.format(svname, bond)
    # Calculate and Plot ...
    probe_arr = generate_probe(origin_pos, shape, along_bd_id)
    t_start = time.time()
    probe_cut = cal_probe_cut(probe_arr, gal_pos, gal_num, cut_style=cut_style)
    t_end   = time.time()
    print('this took {:.3f}'.format(t_end-t_start))
...
if __name__ == '__main__':
    ...
    chdir(tomo_dir)
    probe_dir_name = '{:d}D_{}_probe_cut_L{:d}'.format(len(band_inp), band_inp, L_step)
    if not path.isdir(probe_dir_name):
        system('mkdir {}'.format(probe_dir_name))
    chdir(probe_dir_name)
    l_end   = time.time()
    ...
    # for i in range(len(origin_array)):
        ...
        # title  = '{} smooth_bond={:d}'.format(svname, bond)
        # # Calculate and Plot ...
        # probe_arr = generate_probe(origin_array[i], shape, along_bd_id)
        # t_start = time.time()
        # probe_cut = cal_probe_cut(probe_arr, gal_pos, gal_num, cut_style=cut_style)
        # t_end   = time.time()
        # print('this took {:.3f}'.format(t_end-t_start))
        ...
        # # Progress Indicator
        # drawProgressBar(float(i+1)/len(origin_array))
    Parallel(n_jobs=10)(delayed(execute_all)(origin_array[i]) for i in range(len(origin_array)))
    p_end   = time.time()
